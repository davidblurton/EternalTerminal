...
BackedWriterWriteState BackedWriter::write(Packet packet) {
  ...
  // limit, signal caller to wait.  The limit is on post-disconnect data
  // (rather than total backup size, which can already be at its cap with
  // already-delivered data) so that a busy session still gets the full
  // disconnect headroom.
  if (socketFd < 0 &&
