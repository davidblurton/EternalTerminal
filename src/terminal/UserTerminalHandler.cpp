...
#define BUF_SIZE (16 * 1024)
  ...
  while (true) {
    ...
    // Only read terminal output when the router can accept it.  If the
    // router's buffer is full (e.g. the client is disconnected and the
    // server has stopped draining), leave the data in the pty so the
